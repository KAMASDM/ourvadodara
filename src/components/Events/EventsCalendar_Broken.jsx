...
const EventsCalendar = ({ className = '' }) => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onValue(eventsRef, (snapshot) => {
      ...
      if (data) {
        const firebaseEvents = Object.keys(data).map(key => {
          ...
          return {
            ...
            attendees: Object.keys(event.registrations || {}).length,
            maxAttendees: event.maxCapacity,
            price: event.ticketTypes?.[0]?.price === 0 ? 'Free' : `₹${event.ticketTypes?.[0]?.price}`,
            organizer: event.organizer?.name || 'Organizer',
            description: event.description,
            image: event.images?.[0] || '/images/events/default.jpg',
            rating: 4.5,
            isPopular: false,
            venue: event.venue,
        ...
        // Combine Firebase events with mock events for demo
        const allEvents = [...firebaseEvents, ...mockEvents];
        setEvents(allEvents.filter(event => event.status === 'published' || !event.status));
      } else {
  ...
  return (
    <div className={`min-h-screen bg-gray-50 dark:bg-gray-900 pb-20 -mx-4 px-4 ${className}`}>
      {/* Header */}
      <div className="bg-white dark:bg-gray-800 shadow-sm border-b dark:border-gray-700 p-4 -mx-4 px-4">
                    ...
                    <div className="flex space-x-2">
                      ...
                      {event.registrationRequired && (
                        <button 
                          ...
                          className="bg-red-600 text-white px-3 py-1 rounded-lg hover:bg-red-700 text-xs flex items-center"
                        >
